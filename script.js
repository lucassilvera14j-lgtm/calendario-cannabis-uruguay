document.addEventListener('DOMContentLoaded', () => {
    ...
    // Se utiliza la base de tu dominio Vercel, asumiendo que el serverless function
    // que procesa las alertas está alojado bajo esta misma URL y escucha en /api/guardar-alerta
    const SERVER_URL = 'https://calendario-cannabis-uruguay-9w4fujk29-totis-projects-82aa6668.vercel.app'; 
    ...
    function getPushSubscription() {
        ...
        return navigator.serviceWorker.ready
            ...
            .then((subscription) => {
                ...
                // Si no hay suscripción, se crea una
                const subscribeOptions = {
                    userVisibleOnly: true,
                    applicationServerKey: urlBase64ToUint8Array(applicationServerKey), 
                };
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (currentMonthPhases) { 
                const moonPhase = currentMonthPhases.find(p => p.date === day);
                if (moonPhase) {
                    dayEl.title = `Fase Lunar: ${moonPhase.phase}`;
                    dayEl.classList.add('moon-phase');
                }
            }
    ...
    // --- 4. LÓGICA DE NOTAS (Modal, LocalStorage y PUSH) ---
            ...
            getPushSubscription().then(subscription => {
                if (subscription && text.length > 0) { // Solo si hay suscripción y texto
                    ...
                    const alertData = {
                        date: noteDate,
                        message: text,
                        subscription: subscription, // Clave Push del usuario
                    };

                    // 🚨 USAMOS LA VARIABLE SERVER_URL AQUÍ (URL de Vercel + /api/guardar-alerta)
                    fetch(`${SERVER_URL}/api/guardar-alerta`, { 
                        method: 'POST', 
                    ...
                    .then(response => {
                        if (response.ok) {
                            showToast(`Alerta programada. El servidor se encargará de enviarla el ${noteDate}. ⏲️`, 'success');
                        } else {
                            showToast('Error: El servidor no programó la alerta.', 'error');
                        }
                    })
                    .catch(err => {
                        console.error('Fallo de red al enviar la alerta:', err);
                        showToast('Fallo de red. El servidor debe estar activo para programar alertas. ⚠️', 'error');
                    });
                } else if (text.length > 0) {
                     showToast('Alerta guardada localmente, pero sin suscripción Push activa.', 'warning');
                }
            });
            // --- FIN LÓGICA PUSH ---
    ...
    function initializeMoonSelectors() {
        ...
        moonMonthSelect.value = currentMonth; 
    }
    
    function searchMoonPhases() {
        const selectedMonth = parseInt(moonMonthSelect.value); // (0-11)
        const selectedYear = moonYearSelect.value;             // (e.g., "2025")
        
        moonResultsEl.innerHTML = '';
        
        const phasesForSelectedMonth = REAL_MOON_PHASES[selectedYear] ? REAL_MOON_PHASES[selectedYear][selectedMonth] : null;

        if (!phasesForSelectedMonth || phasesForSelectedMonth.length === 0) {
