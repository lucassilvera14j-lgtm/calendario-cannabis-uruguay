document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ CLAVE PÚBLICA VAPID PEGADA AQUÍ
    ...
    // Función auxiliar para convertir Base64 a ArrayBuffer (necesario para la API)
    ...
    // Función principal para suscribir al usuario a Notificaciones Push
    function subscribeUser() {
        ...
        navigator.serviceWorker.ready.then((registration) => {
            ...
            // Intenta obtener o crear la suscripción
        ...
        .then((pushSubscription) => {
            ...
            // 🚨 ESTO ES LO QUE DEBE HACER TU SERVIDOR (AQUÍ DEBE ENVIARSE):
            ...
            showToast('Notificaciones Push activadas. ✅ Ahora, el servidor puede enviar mensajes.', 'success');
        })
        .catch((e) => {
    ...
    // Función para registrar el Service Worker al cargar la página
    function registerServiceWorker() {
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                // El Service Worker debe estar en la raíz para tener alcance completo (scope)
                navigator.serviceWorker.register('/service-worker.js')
                .then(function(reg) {
                    console.log('Service Worker registrado con éxito.');
                    // Una vez que el SW está registrado, intentamos suscribir al usuario
                    subscribeUser(); 
                })
                .catch(function(err) {
    ...
    // 6. LÓGICA DE FILTRO DE NUTRICIÓN 
    ...
    // MODIFICACIÓN: FUNCIÓN CORREGIDA PARA LEER LOS SELECTORES DE MES Y AÑO
    function searchMoonPhases() {
        const selectedMonth = parseInt(moonMonthSelect.value); // Obtiene el mes (0-11)
        const selectedYear = parseInt(moonYearSelect.value);   // Obtiene el año
        
        moonResultsEl.innerHTML = '';
        
        if (moonPhasesData.length === 0) {
            moonResultsEl.innerHTML = '<p>No se encontraron fases lunares para la selección. 🌑</p>';
            return;
        }
        
        // Simulación de datos: creamos un objeto de fase para el mes y año seleccionado
        let filteredPhases = moonPhasesData.map(phase => ({
            ...phase,
            // Construimos la fecha real basada en la selección del usuario
            fullDate: new Date(selectedYear, selectedMonth, phase.date) 
        }));
        ...
        filteredPhases.forEach(phase => {
            ...
            // Formateamos la fecha completa para mostrarla:
            const dateStr = phase.fullDate.toLocaleDateString('es-ES', { day: 'numeric', month: 'long' });
        ...
        // Feedback al usuario
        showToast(`Resultados cargados para ${moonMonthSelect.options[selectedMonth].text} de ${selectedYear}.`, 'info');
    }
    ...
    // 🚨 LLAMADA PARA REGISTRAR EL SERVICE WORKER Y SUSCRIBIR AL PUSH
